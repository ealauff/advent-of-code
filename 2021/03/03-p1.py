...
# create map opject that represents gamma (the most common bit for each column)
...
# DUMB 3 LINE SOLUTION
# f = open(pathlib.Path(__file__).parent / 'input.txt', "r").read().splitlines()
# gamma = ''.join(list(map(lambda x: '1' if (x > 500) else '0', [sum(x) for x in zip(*list(map(lambda x: list(map(int, list(x))),f)))])))
# print(int(gamma,2)*(~int(gamma,2) & 2**len(f[0])-1))

# DUMB 1 LINE SOLUTION
#print(int(''.join(list(map(lambda x: '1' if (x > 500) else '0', [sum(x) for x in zip(*list(map(lambda x: list(map(int, list(x))),open(pathlib.Path(__file__).parent / 'input.txt', "r").read().splitlines())))]))),2)*(~int(''.join(list(map(lambda x: '1' if (x > 500) else '0', [sum(x) for x in zip(*list(map(lambda x: list(map(int, list(x))),open(pathlib.Path(__file__).parent / 'input.txt', "r").read().splitlines())))]))),2) & 2**len(open(pathlib.Path(__file__).parent / 'input.txt', "r").read().splitlines()[0])-1))

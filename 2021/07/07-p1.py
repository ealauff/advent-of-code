...
# Your puzzle answer was XXXXXX.

###########################################################################################################################################################################

import pathlib

f = open(pathlib.Path(__file__).parent / 'input.txt', "r").read().split(',')
...
for pos in crabs:
    for alignment in range(len(total_fuel)):
        total_fuel[alignment] += abs(pos - alignment)

print(min(total_fuel))

# Pretty slow O(m*n) implementation
